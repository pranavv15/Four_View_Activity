...
function scatter_plot(data,
    ...
    margin = 100)
{
    let fordata = formatted_data(data,
                    x = xLabel,
                    y = yLabel,
                    r = R,
                    c = colour,
...
let xScale= d3.scaleLinear().domain(d3.extent(fordata,function (d){return d.x}))
   .range([margin,1000-margin])
let yScale= d3.scaleLinear().domain(d3.extent(fordata,function (d){return d.y})).range([1000-margin,margin])
let rScale= d3.scaleLinear().domain(d3.extent(fordata,function (d){return d.r})).range([4,12])
